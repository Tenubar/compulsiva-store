...
export const getImageUrl = (path: string | undefined): string => {
    ...
    if (path.includes("/api/images/")) {
      return `${import.meta.env.VITE_SITE_URL}${path.startsWith("/") ? "" : "/"}${path}`
    }
  
    // If the path is just a filename, construct the full API URL
    if (!path.includes("/")) {
      return `${import.meta.env.VITE_SITE_URL}/api/images/${path}`
    }
  
    // For paths that include the filename with directory structure
    const filename = path.split("/").pop()
    if (filename) {
      return `${import.meta.env.VITE_SITE_URL}/api/images/${filename}`
    }
